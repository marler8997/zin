...
fn registerWindowClass(
    ...
    switch (window_config) {
        .static => {
            // if (global.static_callbacks[@intFromEnum(window_id)] != null) std.debug.panic(
            //     "window {s} class has already been registered",
            //     .{@tagName(window_id)},
            // );
            // global.static_callbacks[@intFromEnum(window_id)] = def.callback;
...
pub fn staticWindow(window_id: zin.StaticWindowId) type {
    return struct {
        ...
        pub fn create(opt: zin.CreateWindowOptions) zin.CreateWindowError!void {
            ...
            try createWindow(size, bg, global.connection.staticWindowId(window_id));
...
fn windowSizeFromInit(init: zin.WindowSizeInit) zin.Size {
    return switch (init) {
        .default => @panic("todo"),
        .client => |s| s,
        .window => |s| s,
    ...
    return struct {
        ...
        pub fn clear(self: *const Self) void {
            ...
            x11.clear_area.serialize(messages[after_change_gc..].ptr, false, self.window, .{
                .x = 0,
                .y = 0,
                .width = self.client_size.x,
                .height = self.client_size.y,
            });
        ...
        pub fn rectLtrb(self: *const Self, left: i32, top: i32, right: i32, bottom: i32, rgb: zin.Rgb8) void {
